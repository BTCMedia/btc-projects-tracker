...
def fetch_and_commit
	...
	# Need to check the last commit we've created and see if it matches the most recent here
	latest_release_node_id = releases.first.node_id
	latest_commit_message = client.commits(repo_name).first[:commit][:message]

	unless latest_commit_message.include? latest_release_node_id
		...
		commit_msg = "Latest release: #{latest_release_node_id}"
		sha_new_commit = client.create_commit(repo_name, commit_msg, sha_new_tree, sha_latest_commit).sha
		updated_ref = client.update_ref(repo_name, ref, sha_new_commit)

		puts commit_msg
	else
		puts "No new releases to update"
	end
end
